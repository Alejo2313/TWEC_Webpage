<?php
    $_PNAME ="Inicio";
    $_ACTIVE ="/";
    include "layout/header.php";
?>



<!-- carrusel y botonera -->
<div class="container-fluid">
    <div id="myCarousel" class="carousel slide hidden-xs" data-ride="carousel">
        <!-- Indicators -->
        <ol class="carousel-indicators">
            <li data-target="#myCarousel" data-slide-to="0" class="active"></li>
            <li data-target="#myCarousel" data-slide-to="1"></li>
            <li data-target="#myCarousel" data-slide-to="2"></li>
        </ol>

        <!-- Wrapper for slides -->
        <div class="carousel-inner" role="listbox">
            <div class="item active">
                <img class="carrousel-img" src="./img/img2.png" alt="Image">
                <div class="carousel-caption">
                    <h3>Repara tu movil</h3>
                    <p>con la mejor calidad </p>
                </div>
            </div>

            <div class="item">
                <img class="carrousel-img" src="./img/img.jpg" alt="Image">
                <div class="carousel-caption">
                    <h3>Reparación de Placas base</h3>
                    <p>¿roto?, ¿mojado?, ¡no hay problema!</p>
                </div>
            </div>
            <div class="item">
                <img class="carrousel-img" src="./img/img1.jpg" alt="Image">
                <div class="carousel-caption">
                    <h3>Reparación de ordenadores</h3>
                    <p>hardware, software... ¡lo que quieras!</p>
                </div>
            </div>
        </div>

        <!-- Left and right controls -->
        <a class="left carousel-control" href="#myCarousel" role="button" data-slide="prev">
            <span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span>
            <span class="sr-only">Previous</span>
        </a>
        <a class="right carousel-control" href="#myCarousel" role="button" data-slide="next">
            <span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span>
            <span class="sr-only">Next</span>

        </a>
    </div>
    <div class="container">
        <div class="row form-inline">
            <div class="col-xs-5 col-xs-offset-1 col-sm-3 col-sm-offset-2 col-md-2 col-md-offset-3 text-center offer">
                <button class="btn btn-secondary btn-md btn-block"><span class="glyphicon glyphicon-star-empty"></span>  Destacado</button>
            </div>
            <div class="col-xs-5 col-sm-3  col-md-2 text-center offer ">
                <button class="btn btn-secondary btn-md btn-block"><span class="glyphicon glyphicon-tags"></span>  Novedades</button>
            </div>
            <div class="col-xs-5 col-xs-offset-4  col-sm-3 col-sm-offset-0 col-md-2 col-md-offset-0 text-center offer">
                <button class="btn btn-secondary btn-md btn-block"><span class="glyphicon glyphicon-euro"></span>  Ofertas</button>
            </div>
        </div>
    </div>
    <h2 class="text-center">Productos destacados</h2>
    <p></p>
</div>
<!-- productos destacados -->
<div class="container">
    <div class="row">
        <div class="col-xs-6 col-sm-4 standUp">
            <div class="panel panel-login ">
                <div class="panel-heading">
                    Reparación de Iphone
                    <hr>
                </div>
                <div class="panel-body">
                    <img src="./img/img3.jpg" class="img-responsive" alt="Image">
                </div>
                <div class="panel-footer">
                    <p>Reparación de pantallas, botones, carcasas...Piezas de la mejor calidad</p>
                </div>
            </div>
        </div>
        <div class="col-xs-6 col-sm-4 standUp">
            <div class="panel panel-login ">
                <div class="panel-heading">
                    Reparación de placas
                    <hr>
                </div>
                <div class="panel-body">
                    <div class="crop">
                        <img src="./img/img4.jpg" class="img-responsive" alt="Image">
                    </div>
                </div>
                <div class="panel-footer">
                    <p>Reparación de todo tipo de dispositivos electronicos. ¿mojado?, no hay problema.</p>
                </div>
            </div>
        </div>
        <div class="col-xs-6 col-sm-4 standUp">
            <div class="panel panel-login ">
                <div class="panel-heading">
                    Ordenadores a medida
                    <hr>
                </div>
                <div class="panel-body">
                    <img src="./img/img6.jpg" class="img-responsive" alt="Image">
                </div>
                <div class="panel-footer">
                    <p>Te construimos tu ordenador ajustado a tus necesidades y a tu bolsillo.</p>
                </div>
            </div>
        </div>

        <div class="col-xs-6 col-sm-4 standUp">
            <div class="panel panel-login  ">
                <div class="panel-heading">
                    Componentes
                    <hr>
                </div>
                <div class="panel-body">
                    <img src="./img/img5.jpg" class="img-responsive" alt="Image">
                </div>
                <div class="panel-footer">
                    <p>Venta de componentes de la mejor calidad y al mejor precio.</p>
                </div>
            </div>
        </div>

        <div class="col-xs-6 col-sm-4 standUp">
            <div class="panel panel-login  ">
                <div class="panel-heading">
                    Móviles
                    <hr>
                </div>
                <div class="panel-body">
                    <img src="./img/img7.jpg" class="img-responsive" alt="Image">
                </div>
                <div class="panel-footer">
                    <p>Los mejores moviles y complementos de todas las marcas. </p>
                </div>
            </div>
        </div>


        <div class="col-xs-6 col-sm-4 standUp">
            <div class="panel panel-login  ">
                <div class="panel-heading">
                    <p>Asesoramiento técnico</p>
                    <hr>
                </div>
                <div class="panel-body">
                    <img src="./img/img8.jpg" class="img-responsive" alt="Image">
                </div>
                <div class="panel-footer">
                    <p>Consigue siempre el producto que mejor se adapte a ti. Elige siempre lo mejor</p>
                </div>
            </div>
        </div>
    </div>
</div>



<?php
    include "layout/footer.php"
?>